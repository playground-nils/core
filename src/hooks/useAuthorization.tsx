...
export const useAuthorization = () => {
  ...
  const checkUserIsOrganizationOwner = (
    user: IUser,
  ...
  const checkIsOrganizationMember = (
    user: IUser,
    organization: IAnyOrganization,
  ) => {
    for (let i = 0; i < organization.members.length; i++) {
  ...
  const checkOrganizationsMembership = (
    ...
    if (organizations) {
      ...
      if (!isAllowed) {
        goToApplicationStateError(
          `Check organizations membership fails for organization handle: ${organizationHandle} and organizations: ${organizations}`,
        );
      }
    }
  };
