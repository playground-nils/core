...
function SubNavProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <SubNavContext.Provider value={value}>
      ...
      <div
        className={styles['SubNav__anchor-menu-outer-container']}
        ref={anchoredNavOuterPortalRef}
      >
        <div
          className={clsx(styles['SubNav__anchor-menu-container'])}
          ref={anchoredNavPortalRef}
        />
      </div>
      ...
      >
        <SubNavProvider>
          ...
          >
            <div
              ref={rootRef}
              className={styles['SubNav--header-container-outer']}
            >
              <div className={styles['SubNav__header-container']}>
                {HeadingChild && (
                  <div className={styles['SubNav__heading-container']}>
                    {HeadingChild}
                  </div>
                )}
                {/*
              ...
              */}
                {!isLarge && (
                  <button
                    ...
                    onClick={
                      isOpenAtNarrow ? closeMenuCallback : handleMenuToggle
                    }
                  ...
                  >
                    ...
                    <span
                      className={clsx(
                        ...
                        !activeLinklabel &&
                          styles['SubNav__overlay-toggle-content--end'],
                      )}
                    >
                      {activeLinklabel && (
                      ...
                      {isOpenAtNarrow ? (
                        <ChevronUpIcon
                          className={styles['SubNav__overlay-toggle-icon']}
                          size={24}
                        />
                      ) : (
                        <ChevronDownIcon
              ...
              {LinkChildren.length && (
                ...
                >
                  {ActionChild ? (
                    <li className={styles['SubNav__action-container']}>
                      {ActionChild}
                    </li>
                  ) : (
                    <li className={styles['SubNav__action-container']}></li>
                  )}
                  {LinkChildren}
      ...
      >
        ...
        >
          <Text
            ...
            variant={
              ariaCurrent === 'page' || variant === 'default'
                ? 'default'
                : 'muted'
            }
          >
        ...
        {isLarge && (
          ...
          >
            <ChevronDownIcon
              className={styles['SubNav__sub-menu-icon']}
              size={16}
            />
          </button>
        )}
...
>((props, ref) => {
  ...
  useEffect(() => {
    ...
    const handleIntersectionUpdate: IntersectionObserverCallback = ([
      entry,
    ]) => {
  ...
  if (variant === 'anchor' && context?.portalRef.current) {
    return createPortal(
      <nav
        ref={navRef}
        className={clsx(
          styles['SubNav__sub-menu'],
          styles['SubNav__sub-menu--anchor'],
          className,
        )}
        role="navigation"
    ...
    return (
      <Tag {...(isLarge ? { colorMode: 'light' } : {})}>
        <ul
          className={clsx(
            styles['SubNav__sub-menu'],
            styles[`SubNav__sub-menu--${variant}`],
            className,
          )}
          {...props}
        >
